package com.github.wzt3309.dss.ga.manager;
...
public class TaskManager {
	...
	public static void printTasks(List<Task> tasks,int type){
		BaseLogger.INFO("------------------打印Task列表 开始------------------");
		for(Task task:tasks){
			switch(type){
			case 0:
				BaseLogger.INFO(task);
				break;
			case 1:
				BaseLogger.INFO(task.pCpu());
				break;
			case 2:
				BaseLogger.INFO(task.pMem());
				break;
			case 3:
				BaseLogger.INFO(task.pDisk());
				break;
			case 4:
				BaseLogger.INFO(task.pNet());
				break;
			}
		}
		BaseLogger.INFO("------------------打印Task列表 结束------------------\n");
	}
